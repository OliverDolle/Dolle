...
| --- | --- | --- |
| **agent-development** | AI agents & LLM features — **LangChain + LangGraph** together, system prompts, evals (LLM-as-judge, CI gates, tracing), and voice (STT/TTS) | `langchain-agents`, `langgraph-workflows`, `combining-langchain-and-langgraph`, `workflow-design`, `prompt-engineering`, `speech-to-text`, `text-to-speech`, `eval-foundations`, `llm-as-judge`, `eval-harness-ci`, `langgraph-workflow-evals`, `tracing-observability`, `troubleshooting` |
| **design** | Any interface — web pages via the bundled [Dolle-MCP](https://github.com/OliverDolle/Dolle-MCP) server, native/desktop GUIs (Qt, GTK, WinUI), the UI craft under both (anti-slop, page structure, type & color, surfaces, motion, charts), design systems, and Core Web Vitals | `ui-fundamentals`, `anti-slop`, `structural-variety`, `type-and-color`, `surfaces-and-details`, `motion-and-interaction`, `data-visualization`, `design-systems`, `web-dolle-mcp`, `desktop-native`, `web-performance` |
| **shipping** | Packaging & deploying — Docker & Compose, Kubernetes and GitOps rollouts, CI/CD (GitHub Actions, Azure DevOps) and Terraform | `containerization`, `kubernetes`, `kubernetes-gitops`, `cloud-infrastructure`, `github-actions`, `azure-devops` |
| **engineering** | The code itself — systematic debugging, extensible architecture, database schema and operations, ESP32 firmware | `systematic-debugging`, `extensible-architecture`, `data-modeling`, `database-operations`, `esp32` |
| **process** | How to run the work — sharpening a vague ask, turning an app idea into a build-ready spec, decomposing across subagents and writing their briefs, and a documentation method | `prompt-enhancement`, `app-prompt`, `subagents`, `subagent-briefs`, `documentation` |
...
Nothing but the five descriptions is in context until a hub is invoked — by you, or by Claude matching
your request against a hub description ("my pod is crashing", "review my UI", "write a Dockerfile",
"why is this test flaky" all match on their own). Run `/devkit` any time for the menu.
...
- **Starter templates** (`plugins/devkit/templates/`) — runnable LangGraph and LangChain
  skeletons, a promptfoo eval-in-CI gate, and keyless CI/CD starters, which `/scaffold` copies in and
  adapts.
