...
name: app-prompt
description: >-
  Method for turning a rough app idea into a complete, unambiguous, build-ready specification that a
  downstream implementing agent (Claude Code, another dev, a builder subagent) can execute without
  guessing. Load before building an application from a vague brief. Like the ui-ux-design brief but
  for the whole app: runs a structured AskUserQuestion interview across the axes that decide an app
  (type/platform, users/auth, features/MVP scope, data, integrations, stack, non-functional,
  deployment, success criteria), then compiles a clean, sectioned spec with a phased build order and
  explicit handoff. Specializes the prompt-enhancement method to application creation.
---
...
**Load `devkit:prompt-enhancement` too (or keep it in mind)** — it owns the ask-vs-assume
...
8. **Non-functional requirements.** Expected scale/load; performance targets; security/privacy/
   compliance (auth hardening, GDPR, PCI); accessibility (route UI work to `devkit:ui-design` +
   `devkit:ui-ux-design`/`devkit:gui-design`); internationalization; offline/resilience.
...
9. **UI/UX direction (if it has a UI).** Capture just enough to route and to brief the UI work —
   ...
   `devkit:ui-ux-design` (web) or `devkit:gui-design` (desktop), which run their own design brief.
...
10. **Deployment, ops & lifecycle.** How it's shipped and run — local only, Docker, a cloud target,
    ...
    `devkit:containerization` / `devkit:kubernetes` / `devkit:cloud-infrastructure` when relevant.
...
devkit:ui-ux-design for web, devkit:gui-design for desktop),
...
- `devkit:prompt-enhancement` — the general clarify-then-sharpen method this specializes; read it
...
- `devkit:ui-ux-design` / `devkit:gui-design` — the UI design briefs this spec hands off to (web /
  native). `devkit:ui-design` for the underlying craft.
- `devkit:agent-development` — when the app *is* an AI agent/workflow, the builder loads this.
- `devkit:containerization` / `devkit:kubernetes` / `devkit:cloud-infrastructure` — the deployment
  targets §11 routes to.
- `devkit:subagents` — this spec is exactly the kind of no-ambiguity brief a subagent needs; the
  same bar applies.
