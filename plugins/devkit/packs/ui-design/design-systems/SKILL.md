...
name: design-systems
description: >-
  How a professional stands up a design system so UI decisions are made once and reused everywhere.
  Load when the work outlives a single screen — a product, a component library, more than one
  designer/engineer, a rebrand, or a new dark mode. Covers the three-tier token architecture
  (primitive → semantic → component), building the color/type/spacing/elevation/motion scales into
  tokens, theming (light/dark, multi-brand, density), the component library (variants vs states,
  composition, one canonical build), documentation & governance to stop drift, and the
  design-to-dev handoff where tokens are the shared contract. The setup the fundamentals skill
  scales up to.
---

# Design systems (the professional setup)

The `fundamentals` skill gets *one* screen right. A design system is what makes that decision
**once and reusable** — so the tenth screen, the second engineer, and the dark theme all come out
consistent without anyone re-deciding. This skill is the *setup*: the token architecture, the
...
- `devkit:gui-design` — the same token/component thinking on native toolkits (Qt QSS variables,
  GTK CSS, platform theme APIs).
- `devkit:web-performance` — tokenizing motion/elevation is also where you enforce the
