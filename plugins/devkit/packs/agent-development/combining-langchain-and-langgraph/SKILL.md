---
name: combining-langchain-and-langgraph
description: How LangChain and LangGraph fit together when building agents — the mental model, which to reach for when, and a combined end-to-end example. Read alongside the langchain-agents and langgraph-workflows skills.
---
...
- **Version skew:** keep `langchain*` and `langgraph` versions compatible; mismatches surface as
  import or message-format errors (see the `troubleshooting` skill).
