...
This reference is the counter-pressure. Its claim is narrow and strong: **when you catch one of these
tells in your own draft, that is not a matter of taste — it is the signal that you defaulted, and the
fix is to re-seed the decision from the subject** (`ui-fundamentals.md` §0).
...
the report format. This reference is tool-agnostic — the tells show up in CSS, Figma, Tailwind, SwiftUI
and Qt alike.
...
- **Emoji as UI icons** and **mixed icon libraries** — ✨🚀✅ as feature icons, or Material + Heroicons
  + Lucide on one page. *Fix:* one icon set, one grid, one stroke weight (`ui-fundamentals.md` §12).
...
- **Input geometry that shifts** — `border-width` changing between states, a focus ring built from
  ...
  when empty, disabled signalled by opacity alone. See `ui-fundamentals.md` §7 and
  `motion-and-interaction`.
...
- **`ui-fundamentals.md` §0** — the aesthetic-direction rules this reference enforces: ground it in
  the subject, hero-as-thesis, structure carries meaning, spend boldness once.
- **`structural-variety`** — the positive answer to §2's structural tells: named page
  shapes, nav/footer archetypes, hero fit, and the don't-repeat-yourself rule.
- **`type-and-color`** — the positive answer to the type and palette tells: real pairings,
  OKLCH palettes, accent discipline, and the contrast pairs that fail most often.
- **`motion-and-interaction`** — recipes and canon behind §4's motion tells.
- `web-dolle-mcp.md` — run this sweep against the screenshot when building on Dolle-MCP.
- `web-performance.md` — the LCP/CLS items in §2 and §6 are the same rules measured.
