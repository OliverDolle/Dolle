...
This reference is those two. It is tool-agnostic; the CSS is illustration, not requirement.
...
| 1 | **Whitespace** | Proximity is the grouping (`ui-fundamentals.md` §1). Cheapest, quietest, usually right. |
...
- Media: `width`/`height` or `aspect-ratio` on every image and embed (CLS — `web-performance.md`).
...
- **`ui-fundamentals.md` §2/§5** — the spacing scale these paddings come from and the state set
  the focus/hover details finish.
- **`anti-slop`** — the container tells (card-in-card, side-stripe card, shadow-glow on
  dark, icon-tile card, glassmorphism-as-decoration) and the implementation tells (§6) this reference is
  the positive answer to.
- **`type-and-color`** — surface/ink tokens, the tinted-neutral rule, and the surface-flip
  contrast rule referenced in §6.
- **`design-systems`** — promote the radius, rule, shadow, surface and density decisions to
  tokens so a whole product shares one containment language.
- `web-performance.md` — the reserved-space items in §7 are CLS, measured.
- `desktop-native.md` — on native platforms the *platform* owns radius, elevation and system colors;
