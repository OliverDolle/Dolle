...
This reference makes page shape an explicit, stated decision, made *before* markup. It is tool-agnostic:
...
- **`anti-slop`** — the tells this reference routes around (the AI nav, the AI footer, the
  centered `100vh` hero, the three-up grid, decoration without purpose) and the gate sweep.
- **`ui-fundamentals.md` §0/§1/§9** — *structure carries meaning*, *compose don't stack*, and the
  hierarchy/responsive rules the shape has to satisfy.
- **`type-and-color`** — the surface that sits on the shape; the diversification axes in §3
  reference its palette and display-voice vocabulary.
- `web-dolle-mcp.md` — the Dolle-MCP build workflow; its template catalog is where these shapes
  get built, and its brief step is where you offer the trio.
- `web-performance.md` — hero LCP and CLS are decided by the §6/§7 choices.
