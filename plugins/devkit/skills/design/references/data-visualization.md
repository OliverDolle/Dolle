...
could. This reference is the craft of getting the answer across at a glance: picking the mark from
...
Dolle-MCP chart templates) and goes *deeper* than `ui-fundamentals.md` on the one thing charts add:
encoding quantity in position, length, and hue.

The neighbors own the rest. `ui-fundamentals.md` owns hierarchy, type, states, and accessibility in
...
- **KPI row → detail.** Top band: the 3–5 headline numbers with their deltas. Middle: the trend and
  ...
  The eye reads top-left first (§1 of `ui-fundamentals.md`) — put the most important tile there.
...
- **Grey is a color.** Mute everything that isn't the point and let one accent carry the finding —
  the same 60/30/10 restraint as `ui-fundamentals.md` §4. Check every text-on-fill pair for AA contrast.
...
A chart is a data view, so it inherits the **four content states** (`ui-fundamentals.md` §6) — design them,
...
- **Loading.** Skeleton the chart area at its final size so the layout doesn't jump; prefer a shaped
  placeholder over a bare spinner. Reserve the space (CLS — `web-performance.md`).
...
- **Don't encode by color alone** (§3) and clear **AA contrast** on text, labels, and against
  adjacent fills (`ui-fundamentals.md` §4).
...
- **`ui-fundamentals.md`** — the general craft charts inherit: hierarchy (§1), semantic color +
  WCAG contrast (§4), the four content states (§6), and accessibility (§11). This reference adds only
  what's specific to encoding data.
- **`design-systems`** — resolve chart colors from the **color tokens/ramps** defined there
  (§2); categorical/sequential/diverging sets belong in the token layer so every chart matches.
- `web-performance.md` — reserving chart space (CLS) and compositor-only, reduced-motion chart
  transitions (INP) are governed by the same rules.
- `web-dolle-mcp.md` — build charts and dashboards on the **Dolle-MCP** server (chart/dashboard
