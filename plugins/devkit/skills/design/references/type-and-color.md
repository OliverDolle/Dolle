...
fails most visibly. `ui-fundamentals.md` §3–§4 sets the baseline (a small scale, semantic roles, AA
contrast). This reference is the depth: **which** faces, **which** values, and the specific pairings that
break.
...
- **`ui-fundamentals.md` §3/§4** — the baseline this deepens (small scale, semantic roles, AA
  contrast, never color alone).
- **`anti-slop`** — the palette and type tells this reference's rules are the answer to, plus
  the gate sweep that checks them.
- **`design-systems`** — turn these values into primitive → semantic → component tokens and
  a theme mapping so the discipline survives many screens.
- **`data-visualization`** — categorical/sequential/diverging chart color resolved from
  these same tokens.
- `web-dolle-mcp.md` — Dolle-MCP's `find_palettes` / `color_palettes` / `color_info` /
