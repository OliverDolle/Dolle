...
landed, and keeps the eye oriented across a change. This reference goes deeper than `ui-fundamentals.md` §8
...
The premise from `ui-fundamentals.md` §8 holds throughout: motion has a job, it's fast, it animates
`transform`/`opacity` only, and it's gated. This reference is *how* to make it good, not permission to
add more of it.
...
- **Trigger** — what starts it (user click/hover/focus, or a system event). Every interactive element
  ...
  `ui-fundamentals.md` §5) — motion makes those states legible, it doesn't replace them.
...
- **One orchestrated moment.** Like `ui-fundamentals.md` §0's "spend boldness once" — pick **one** hero
...
- **Skeleton, then reveal** for content areas: show a skeleton at the final layout's shape and size,
  then cross-fade to real content. This previews structure and cuts perceived wait (`ui-fundamentals.md`
  §5/§6). Reserve the exact space so nothing shifts when data lands (CLS — `web-performance.md`).
...
- **The INP/CLS link.** Motion is a performance surface, not just an aesthetic one. Janky, main-thread
  ...
  rule are the same rule (`web-performance.md`).
...
from §0's three jobs. (Full tell catalog across every dimension: `anti-slop.md`.)
...
- **`ui-fundamentals.md` §8** — the baseline motion rules this reference deepens (fast, eased,
  `transform`/`opacity`, gated); also §5/§6 for the states and skeletons motion animates between.
- **`anti-slop`** — §7's tells in the wider catalog of AI-default giveaways, plus the gate
  sweep that checks them before you ship.
- **`design-systems`** — put duration/easing/spring in the **motion tokens** (§2) and expose
  the reduced-motion alternate as part of the token contract, so every component animates consistently.
- `web-performance.md` — the CLS (reserve space) and INP (compositor-only, off the main thread)
  rules are the performance face of §5/§6 here; measure motion cost there.
- `web-dolle-mcp.md` — apply this motion on the **Dolle-MCP** template build, and screenshot/
