...
- `containerization.md` — the image CI builds, scans, and pushes; the artifact promoted through
  the pipeline.
- `kubernetes.md` — one compute target; provision the cluster with the IaC here and deploy via
  the pipeline (GitOps with Argo CD / Flux is the K8s-native form of Step 3).
- `devkit:design` → `references/web-performance.md` — field metrics (CrUX/RUM) feed the observability picture for anything
  user-facing.
