...
`kubernetes.md` gets a *correct manifest* onto the cluster. This reference is the layer on top:
...
- A correct **readinessProbe** is what makes RollingUpdate safe — a pod joins the Service only when
  ready, so traffic never hits a booting replica. (Probe details live in `kubernetes.md`.)
...
- **Secrets never in plaintext git.** Commit only *encrypted* or *referenced* secrets:
  **SOPS** (encrypt values in-repo, decrypt in-cluster), **Sealed Secrets** (a public-key-encrypted
  ...
  See `kubernetes.md` for why a raw `Secret` is only base64.
...
- Roll back the *artifact*, not by hot-patching live pods (that's drift). Then diagnose with the
  `kubernetes.md` pod-debugging loop.
...
- `kubernetes.md` — the manifests this reference ships: Deployments/Services/Ingress,
...
- `github-actions.md` / `azure-devops.md` — the CI that *builds and pushes the
  image and opens the promotion PR* feeding this delivery layer.
- `cloud-infrastructure.md` — provisioning the cluster and the vendor-neutral CI/CD and
  twelve-factor baseline this builds on.
- `containerization.md` — producing the immutable, digest-addressed image that gets promoted.
