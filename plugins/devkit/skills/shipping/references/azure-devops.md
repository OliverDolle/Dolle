...
Carry the `cloud-infrastructure.md` rule: **build once, promote the same artifact** across stages via
...
- **Scan in-pipeline:** image CVEs (Trivy/Defender), dependency audit, IaC scan
  (`checkov`/`tfsec`), secret scan (gitleaks) — same stages as `cloud-infrastructure.md` Step 3.
...
- `containerization.md` — the image the Build stage builds, scans, and pushes.
- `kubernetes.md` — a common deploy target; the `canary`/rolling manifests the deployment job
  applies.
