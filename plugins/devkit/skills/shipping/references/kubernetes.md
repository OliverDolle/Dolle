...
- `containerization.md` — the image these manifests run: its non-root user, `HEALTHCHECK`, and
  `SIGTERM` handling are what the securityContext, probes, and graceful termination here depend on.
- `cloud-infrastructure.md` — provisioning the cluster (Terraform/EKS/GKE/AKS), CI that builds
