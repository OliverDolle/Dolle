...
lives in `cloud-infrastructure.md` — read it first. This is the field manual for the YAML.
...
Key rule carried from `cloud-infrastructure.md`: **build the artifact once, promote the same digest**;
...
| --- | --- |
| **Rolling** | `kubectl rollout` / `helm upgrade` (K8s does the gradual replace), or ECS rolling update. See `kubernetes.md`. |
...
- `cloud-infrastructure.md` — the vendor-neutral pipeline/IaC/OIDC/observability concepts this
  field manual makes GitHub-concrete. Prerequisite.
- `containerization.md` — the image this workflow builds, scans, pushes, and attests.
- `kubernetes.md` — a common deploy target; `kubectl rollout`/Helm execute the rollout the
  deploy job triggers.
