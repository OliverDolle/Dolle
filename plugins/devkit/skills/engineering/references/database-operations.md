...
migrations, bounded connections, and a restore you've actually tested. This reference keeps the
...
- **No secrets in code or images** (see `devkit:shipping` → `references/containerization.md` — runtime injection, not `ENV`).
...
- `data-modeling.md` — the companion reference: the schema, keys, and FKs this one indexes, migrates, and
  locks. Read it first if the schema isn't settled.
- `devkit:shipping` → `references/cloud-infrastructure.md` — provisions the managed DB (backups, IAM/OIDC auth, secret store,
  networking) as IaC; this reference decides the config it should hold.
- `devkit:shipping` → `references/containerization.md` — runs Postgres locally for dev and injects credentials at runtime, not
  in image layers.
- `devkit:shipping` → `references/kubernetes.md` — where the app runs against the DB; prefer a managed service over
