...
- `subagents.md` — once a prompt is sharp, a self-contained brief is exactly what a subagent
  needs; the same "no ambiguity left" bar applies to the brief you hand it.
- `devkit:design` → `references/web-dolle-mcp.md` — its design brief *is* prompt enhancement applied to a design task (asking
