...
evaluated (§4). This reference designs and validates the judge; deciding *what* to score is
...
- `eval-foundations.md` — decide which criteria need a judge at all.
- `eval-harness-ci.md` — the `llm-rubric` assertion and mixing judge + deterministic checks.
- `tracing-observability.md` — run the judge online over sampled production traffic.
