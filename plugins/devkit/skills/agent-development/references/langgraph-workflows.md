...
LangGraph builds on LangChain — its models and `@tool`s (see `langchain-agents.md`) plug
straight in.
...
- Branching, cycles, retries, memory across turns, approval steps, or multiple cooperating
  agents → LangGraph (this reference). Start from `create_react_agent` and grow into a custom
