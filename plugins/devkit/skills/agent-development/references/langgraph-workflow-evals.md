...
This is the **LangGraph-specific** application of the eval references. They own the
...
This reference maps those onto a concrete `StateGraph`: how to turn the graph you built with
`langgraph-workflows` into an eval harness. Read the siblings for the
...
`llm-as-judge.md` wholesale: binary per criterion, structured output + reason,
...
- **Run as a required PR check**, with API keys via CI secrets/OIDC (`devkit:shipping` → `references/cloud-infrastructure.md`).
...
- `eval-foundations.md` — what to measure; task/component/trajectory levels this instantiates.
- `llm-as-judge.md` — how to design/validate the LLM graders used for node + trajectory judging.
- `eval-harness-ci.md` — the pass-rate merge gate this ties graph changes into.
- `tracing-observability.md` — capture the span tree these trajectory evals score.
- `langgraph-workflows.md` — build the StateGraph, checkpointer, and tools you're evaluating here.
