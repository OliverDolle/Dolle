...
- `devkit:process` → `references/prompt-enhancement.md` — the upstream half: turn a vague *user request* into a sharp task. Clarify
  there, then engineer the prompt here.
- `devkit:process` → `references/app-prompt.md` — same craft applied to turning an app idea into a build-ready spec.
- `devkit:process` → `references/subagent-briefs.md` — a subagent brief is a prompt; the
  structure and "no ambiguity left" bar here apply directly to worker prompts.
- `eval-foundations.md` / `eval-harness-ci.md` — §7's eval-driven loop is how you validate agent and
