...
- `speech-to-text.md` — the other half; pair for a full voice loop.
- `langchain-agents.md` / `langgraph-workflows.md` — stream LLM tokens into TTS so replies start speaking sooner.
- `devkit:process` → `references/app-prompt.md` — settle voice persona, languages, on-device vs cloud, and any
