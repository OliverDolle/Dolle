...
- `text-to-speech.md` — the other half; pair for a full voice loop.
- `langchain-agents.md` / `langgraph-workflows.md` — the reasoning loop between STT and TTS in a voice agent.
- `devkit:process` → `references/app-prompt.md` — settle languages, on-device vs cloud, privacy/consent, and latency budget
