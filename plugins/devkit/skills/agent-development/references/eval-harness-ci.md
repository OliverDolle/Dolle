...
gate, LLM changes ship on vibes and regress silently. This reference turns the dataset from
...
devkit ships the `promptfoo-eval-ci` template (`${CLAUDE_PLUGIN_ROOT}/templates/promptfoo-eval-ci/`) — `/scaffold promptfoo-eval-ci` copies it in to get a working gate fast.
It contains:
...
(`devkit:shipping` → `references/cloud-infrastructure.md` covers secrets), and set the check as required.
...
- `eval-foundations.md` — the dataset and levels this gate runs on.
- `llm-as-judge.md` — designing/validating the `llm-rubric` assertions.
- `tracing-observability.md` — capture traces for tool-call assertions; online eval.
- `devkit:shipping` → `references/cloud-infrastructure.md` — run the harness as a pipeline stage; OIDC + secrets for API keys.
- Scaffold: `/scaffold promptfoo-eval-ci` copies the template in.
