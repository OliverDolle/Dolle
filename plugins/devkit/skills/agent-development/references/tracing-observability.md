...
(§3) runs graders over live traces, and drift monitoring (§4) alerts on them. This reference
...
- `eval-foundations.md` — trajectory metrics + offline/online split these traces feed.
- `llm-as-judge.md` — the graders run online over sampled traffic.
- `eval-harness-ci.md` — offline gate; drift findings become regression cases.
- `devkit:shipping` → `references/cloud-infrastructure.md` — its observability guidance; export traces to your backend.
- `langchain-agents.md` / `langgraph-workflows.md` — the agents/workflows you're instrumenting.
